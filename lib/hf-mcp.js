...
// Un resultat d'outil MCP arrive en blocs de contenu ; les outils Higgsfield
// repondent du JSON encode en texte.
function resultJson(result) {
    const block = result && Array.isArray(result.content) ? result.content.find(c => c.type === 'text') : null;
    if (!block) return null;
    try { return JSON.parse(block.text); } catch (e) { return { raw: block.text }; }
}

module.exports = { callTools, listTools, resultJson, getAccessToken };

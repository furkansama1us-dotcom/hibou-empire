...
function parseTabular(text) {
    ...
    const headerIndex = lines.findIndex(l => /\[\d+\]\s*\([^)]*\)\s*:/.test(l));
    if (headerIndex === -1) return null;

    const columns = lines[headerIndex].match(/\(([^)]*)\)/)[1].split(',').map(c => c.trim());
    const rows = [];
    for (let i = headerIndex + 1; i < lines.length; i++) {
        const line = lines[i].trim();
        if (!line) continue;
        if (/\[\d+\]\s*\([^)]*\)\s*:/.test(line)) break; // un autre tableau commence
